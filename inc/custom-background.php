...
add_action( 'after_setup_theme', 'rcdoc_custom_background_setup', 15 );

# Filter the background color late.
add_filter( 'theme_mod_background_color', 'rcdoc_background_color', 95 );
 ...
 * @return void
 */
function rcdoc_custom_background_setup() {
	...
	add_theme_support(
		...
		array(
			...
			'default-image'    => '',
			'wp-head-callback' => 'rcdoc_custom_background_callback',
		)
	);
}
 ...
 * @return string
 */
function rcdoc_background_color( $color ) {
 ...
 * @return void
 */
function rcdoc_custom_background_callback() {
